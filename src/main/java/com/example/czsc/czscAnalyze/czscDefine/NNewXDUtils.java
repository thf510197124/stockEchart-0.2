...
import com.example.czsc.czscAnalyze.czscDefine.toolsForXD.DecopyList;
 ...
 * @History: 2021/5/2
 */
public class NNewXDUtils {
    private static List<BI> originalBI;
    //定义为DecopyList，是的获得的是一个与originalBI无关的笔，即使相等，但不是同一个对象
    private static final DecopyList unhandled_BI = new DecopyList();//一次上涨，或者一次下跌形成结束后变动
    ...
    private static XD handleUpXD(List<BI> bis) {
        ...
        splitAspectBI(split);
        println("处理完包含关系前，split的时间点位：" + bis.get(split).getEndTime());
        split = handleUpContains(split);
        println("处理完包含关系后，split的时间点位：" + bis.get(split).getEndTime());
        printSplit("--");
        println("处理过包含关系后，unhandled_BI 为");
        ...
        if(!hasGap(aspectBI1,aspectBI2)){//破坏线段形成已经经过判断，所以，如果没有缺口，则直接判断上涨线段结束
            ...
            DecopyList afterBI = new DecopyList(subBISFromOriginal(unhandled_BI.get(index).getEndTime()));
            ...
            int lowPointIndex = afterBI.indexOf(afterBI.getBIByTime(interval.getEndTime()));
    ...
    private static XD handleDownXD(List<BI> bis) {
        ...
        splitAspectBI(split);
        printSplit("--");
        println("处理完包含关系前，unhandled_BI中的数量为：" + unhandled_BI.size());
        split = handleDownContains(split);

        println("处理过包含关系后，unhandled_BI 数量为" + unhandled_BI.size());
        //println(unhandled_BI);
        printSplit("--");
        BI aspectBI1 = getLast(before_aspect_BI);
        BI aspectBI2 = after_aspect_BI.get(0);
        ...
        if(!hasGap(aspectBI1,aspectBI2)){//破坏线段形成已经经过判断，所以，如果没有缺口，则直接判断上涨线段结束
            ...
            int highPointIndex = afterBI.indexOf(afterBI.getBIByTime(interval.getEndTime()));
     ...
     */
    private static int handleUpContains(int splitIndex){
        LinkedList<BI> beforeBI = new LinkedList<>(before_aspect_BI);
        LinkedList<BI> afterBI = new LinkedList<>(after_aspect_BI);
        int origLength = unhandled_BI.size();
        handleUpAspect(beforeBI, before_aspect_BI,UP);
        if (unhandled_BI.size() < origLength){
            splitIndex = splitIndex + unhandled_BI.size() - origLength;
        }
        handlerDownAspect(afterBI, after_aspect_BI,UP);
        /*println("上涨中处理特征线段的包含关系结束，处理后前特征笔有：");
    ...
    //仍是处理后包含，不处理前包含
    private static int handleDownContains(int splitIndex) {
        LinkedList<BI> beforeBI = new LinkedList<>(before_aspect_BI);
        LinkedList<BI> afterBI = new LinkedList<>(after_aspect_BI);
        int origLength = unhandled_BI.size();
    ...
    private static void handleUpAspect(LinkedList<BI> beforeBI,LinkedList<BI> before_aspect_bi,Direction direction) {
        ...
        for (BI value : beforeBI) {
            ...
            if (ship == IntervalRelationship.INCLUDEAFTER) {
                containedBI(before_aspect_bi, direction, interval, value);
            } else if (ship == IntervalRelationship.UP || ship == IntervalRelationship.UNCONTAIN_UP ||
                    ship == IntervalRelationship.ISCONTAINED) {
                interval = value;
            }
    ...
    /**
     * 前后两个方法中抽离的共同代码
     * @param before_aspect_bi
     * @param direction
     * @param interval
     * @param value
     */
    private static void containedBI(LinkedList<BI> before_aspect_bi, Direction direction, BI interval, BI value) {
        int index = before_aspect_bi.indexOf(interval);
        if (direction == UP){
            //当前假线段为上涨，包含的方式为\/\，可以取昨高，右低
            interval.setLow(value.getLow());
        }else{//如果主方向向下，那么包含关系处理为，当前interval设置为取value的低点
            interval.setHigh(value.getHigh());
        }
        interval.setEndTime(value.getEndTime());
        interval.setFx_end(value.getFx_end());
        before_aspect_bi.set(index,interval);
        before_aspect_bi.remove(value);

        BI between = unhandled_BI.get(unhandled_BI.indexOf(value) - 1);
        unhandled_BI.remove(between);
        unhandled_BI.remove(value);
    }
    ...
    private static int foundSplitXDIndex(List<BI> bis) {
        if (bis.size() < 4){
            ...
            BI intervalFirst = bis.get(1);
            for (int i=3;i < bis.size();i= i +2){
                IntervalRelationship ship = IntervalRelationship.getRelationship(interval,bis.get(i));
                if(isDirectionUP){
                    if(ship == IntervalRelationship.UP || ship == IntervalRelationship.UNCONTAIN_UP){//向下突破，重新定义区间
                        interval = bis.get(i);
                    }else if(ship == IntervalRelationship.DOWN || ship == IntervalRelationship.UNCONTAIN_DOWN
                    ){//向下突破，这样就确定形成了向下的暂定线段。
                        //向下突破，可能该笔完全包含前一笔，这种情况，是破坏笔，在前一笔的判断中
                        return unhandled_BI.indexOf(interval) - 1;
                    }else if(ship ==IntervalRelationship.ISCONTAINED){
                        if (bis.get(i).getLow() > intervalFirst.getLow()){
                            interval = bis.get(i);
                        }else{
                            return unhandled_BI.indexOf(bis.get(i)) - 1;//防止出现极端的情况，该笔完全包含前面一个线段的高点和低点
                        }
                    }

                }else{
                    //这里可能出问题，
                    if(ship == IntervalRelationship.DOWN || ship == IntervalRelationship.UNCONTAIN_DOWN){//向上突破，重新定义区间
                        //这里把前包含，不进行处理，看最后突破方向，如果包含后再往上，就确定低点
                        interval = bis.get(i);
                    }else if(ship == IntervalRelationship.UP || ship == IntervalRelationship.UNCONTAIN_UP){//向下突破，这样就确定形成了向下的暂定线段。
                        //向下突破，可能该笔完全包含前一笔，这种情况，是破坏笔，在前一笔的判断中
                        return unhandled_BI.indexOf(interval) - 1;
                    }else if(ship ==IntervalRelationship.ISCONTAINED){
                        if (bis.get(i).getHigh() < intervalFirst.getHigh()){
                            interval = bis.get(i);
                        }else{
                            return unhandled_BI.indexOf(bis.get(i)) - 1;
                        }
                    }
                }
            }
    ...
    private static void trimUnHandled_BI(XD xd){
        ...
        unhandled_BI.clear();
        unhandled_BI.addAll(subBISFromOriginal(time));
        /*println(unhandled_BI);*/
    }
    private static List<BI> subBISFromOriginal(LocalDateTime time){
        List<BI> afterTime = new ArrayList<>();
        for (BI bi : originalBI){
            if (bi.getBeginTime().isAfter(time.minusSeconds(1))){
                afterTime.add(bi);
            }
        }
        return afterTime;
    ...
    /*public static void main(String[] args) throws Exception {
        String path = "/Users/haifeng/Documents/stock_database/baostock/day/600692.csv";
